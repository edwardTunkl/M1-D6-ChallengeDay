...
// console.log(splitMe("hello how are you"))
...
function deleteOne (str, bool) {
  ...
  return bool === true ? str.slice(1) : str.slice(0, str.length-1)
}
...
//  console.log(deleteProp("hey", "hello"))
...
let oldestOne = movies[0].Year
  for (let i=1; i<movies.length; i++ ){
    if(movies[i].Year<oldestOne){
      oldestOne = movies[i].Year
    } else {
      oldestOne
    }
  }
...
for (let i=0; i<movies.length; i++)
  if (id == movies[i].imdbID){
    movieById = movies[i].Title
    break
  } else {
    
    // console.log(`Could not find any move with ${id}`)
  }
  return movieById
}
